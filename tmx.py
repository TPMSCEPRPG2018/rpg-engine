...
class Tile(object):
    ...
    def __repr__(self):
        return '<Tile {gid}>'.format(gid=self.gid)
...
class Layer(BaseLayer):
    ...
    def get_in_region(self, x1, y1, x2, y2):
        ...
        return [self.cells[i, j] for i in range(int(i1), int(i2)) for j in range(int(j1), int(j2)) if
                (i, j) in self.cells]
...
visible: Whether the object is shown (1) or hidden (0). Defaults to 1.
    ...
    def fromxml(cls, tag, map_):
        ...
        o = cls(tag.attrib.get('type', 'rect'), int(tag.attrib['x']), int(tag.attrib['y']), w, h,
...
class ObjectLayer(BaseLayer):
    ...
    def __init__(self, name: str, color: str, objects: List[Object], opacity: float = 1, visible: bool = 1,
                 position=(0, 0)):
        self.name = name
        self.color = color
    ...
    @classmethod
    def fromxml(cls, tag, map_):
        layer = cls(tag.attrib['name'], tag.attrib.get('color'), [], float(tag.attrib.get('opacity', 1)),
                    bool(int(tag.attrib.get('visible', 1))))
    ...
    def collide(self, rect, property_name):
        ...
        return [obj for obj in self.get_in_region(rect.left, rect.top, rect.right, rect.bottom) if
                property_name in obj or property_name in self.properties]
