...
class Tileset(object):
	...
	def add_image(self, file):
		...
		if not image:
			raise FileNotFoundError("Error creating new Tileset: file %s not found" % file)
		id_ = self.firstgid
		for line in range(image.get_height() // self.tile_height):
			for column in range(image.get_width() // self.tile_width):
				pos = Rect(column * self.tile_width, line * self.tile_height, self.tile_width, self.tile_height)
...
class Layer(BaseLayer):
	...
	def __init__(self, name, visible, map_):
		self.name = name
		self.visible = visible
		self.position = (0, 0)
		# TODO get from TMX?
		self.px_width = map_.px_width
		self.px_height = map_.px_height
		self.tile_width = map_.tile_width
		self.tile_height = map_.tile_height
		self.width = map_.width
		self.height = map_.height
		self.tilesets = map_.tilesets
	...
	@classmethod
	def fromxml(cls, tag, map_):
		layer = cls(tag.get('name'), int(tag.get('visible', 1)), map_)
		
		layer.height = int(tag.get('height', layer.height))
		layer.width = int(tag.get('width', layer.width))
		...
		if len(data):
			data = [int(tile.get('gid', 0)) for tile in data]
		else:
		...
		for i, gid in enumerate(data):
			...
			tile = map_.tilesets[gid]
			x = i % layer.width
			y = i // layer.width
			layer.cells[x, y] = Cell(x, y, x * map_.tile_width, y * map_.tile_height, tile)
	...
	def find(self, *properties):
		...
		return [cell for cell in self.cells.values() if cell and all(name in cell for name in properties)]
	
	def match(self, **properties):
		"""Find all cells with the given properties set to the given values."""
		return dict(properties.items() & self.cells.items())
	
	# return [value for key, value in properties.items() if key in self.cells]
	
	def collide(self, rect, propname):
		"""
		Find all cells the rect is touching that have the indicated property name set.
		"""
		r = []
	...
	def get_in_region(self, x1, y1, x2, y2):
		...
		return [self.cells[i, j] for i in range(int(i1), int(i2)) for j in range(int(j1), int(j2)) if (i, j) in self.cells]
...
visible: Whether the object is shown (1) or hidden (0). Defaults to 1.
	...
	def __init__(self, type_, x, y, width=0, height=0, name=None, gid=None, tile=None, visible=1):
		self.type = type_
		self.px = x
		self.left = x
		if tile:
	...
	def __contains__(self, key):
		return key in self.properties or (self.tile and key in self.tile.properties)
	...
	def fromxml(cls, tag, map_):
		...
		o = cls(tag.attrib.get('type', 'rect'), int(tag.attrib['x']), int(tag.attrib['y']), w, h, tag.attrib.get('name'), gid, tile, int(tag.attrib.get('visible', 1)))
...
class ObjectLayer(BaseLayer):
	...
	def __init__(self, name: str, color: str, objects: List[Object], opacity: float = 1, visible: bool = 1, position=(0, 0)):
		self.name = name
		self.color = color
	...
	@classmethod
	def fromxml(cls, tag, map_):
		layer = cls(tag.attrib['name'], tag.attrib.get('color'), [], float(tag.attrib.get('opacity', 1)), bool(int(tag.attrib.get('visible', 1))))
		for obj in tag.findall('object'):
			layer.objects.append(Object.fromxml(obj, map_))
	...
	def find(self, *properties):
		...
		return [obj for obj in self.objects if obj and all(name in obj for name in properties)]
		...
		for property_name in properties:
			for obj in self.objects:
				if property_name in obj:
					...
					val = self.properties[property_name]
				else:
					continue
				
				if properties[property_name] == val:
					r.append(obj)
		return r
	
	def collide(self, rect, property_name):
		"""
		Find all objects the rect is touching that have the indicated property name set.
		"""
		
		return [obj for obj in self.get_in_region(rect.left, rect.top, rect.right, rect.bottom) if property_name in obj or property_name in self.properties]
	
	def get_in_region(self, x1, y1, x2, y2):
		"""
		Return objects that are within the map-space pixel bounds specified by the bottom-left (x1, y1) and top-right (x2, y2) corners.
...
class SpriteLayer(pygame.sprite.AbstractGroup, BaseLayer):
	def __init__(self):
...
class Layers(list):
	def __init__(self):
		super().__init__()
		self.by_name: Dict[str, BaseLayer] = dict()
	
	def add_named(self, layer: BaseLayer, name: str):
		self.append(layer)
		self.by_name[name] = layer
	
	def __getitem__(self, item) -> BaseLayer:
...
class TileMap(object):
	...
	def load(cls, filename, viewport):
		with open(filename) as f:
			map_ = ElementTree.fromstring(f.read())
		
		# get most general map informations and create a surface
		tilemap = TileMap(viewport)
		tilemap.width = int(map_.attrib['width'])
		tilemap.height = int(map_.attrib['height'])
		tilemap.tile_width = int(map_.attrib['tilewidth'])
		tilemap.tile_height = int(map_.attrib['tileheight'])
